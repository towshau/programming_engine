...
interface WcrTableProps {
  rows: WcrRow[]
}

export function WcrTable({ rows }: WcrTableProps) {
  const [sortCol, setSortCol] = useState<string>('submission_date')
  const [sortDir, setSortDir] = useState<SortDir>('desc')

  const columns = useMemo(
    () => columnKeysForRows(rows, ['submission_date', 'coach_name', 'coach_id', 'id'], EXCLUDE),
    [rows],
  )
  ...
  function toggleSort(col: string) {
    if (sortCol === col) {
      ...
      setSortCol(col)
      setSortDir(col === 'submission_date' ? 'desc' : 'asc')
    }
  }
  ...
  if (rows.length === 0) {
    return (
      <p className="text-sm py-4" style={{ color: 'var(--text-muted)' }}>
        No winning client results for this coach filter.
      </p>
    )
  }
    ...
    <div className="overflow-x-auto -mx-1">
      <table className="min-w-full text-sm border-collapse">
        ...
        <tbody>
          {sorted.map((row, i) => (
            <tr
              key={(row.id as string | undefined) ?? `wcr-${i}`}
              className="border-b"
