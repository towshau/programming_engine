...
**Pages:**
- **ClientQueue** (`/`) — landing page. **5** full-width tabs: Awaiting First Program (red), New Training Phase Due (gold), Active Programs (green), **Program Updates** (red badge — members with unactioned **`member_programming_notes`** where `implemented = false`), Holiday Programs & Holds (blue). Member lists are derived from the same `fetchMembers` load (notes attached per member; coach filter unchanged: `member_memberships.handoff_coach_id ?? coach_id`). **Row click (Awaiting / New Training Phase Due / Active / Program Updates):** opens **`MemberDetailModal`** — **Client Profile** + **Movement & Benchmarks**; **Open Program Editor** → `/program/:memberId`. **Program Updates** rows show note count + latest modification type. **Holiday** tab: click → `/holiday`. Shared intake UI: `IntakeCards`, `intakeTypes`, `intakeProfile`. **ProgrammingNotesPanel** (`features/program/ProgrammingNotesPanel.tsx`): collapsible gold bar under **`ProgramHeader`** in **`ProgramViewer`** (and above “no program yet” card when no program) — lists all notes for the member sorted **unactioned first**, then **urgent** (`Injury / Pain`) before **medium** (other modification types), then date; each note shows **Urgent/Medium** pill and **Submitted by {staff_name} on {date}**; checkbox **Mark as implemented** updates `implemented` and refreshes queue via `fetchMembers`.
