...
**Pages:**
- **ClientQueue** (`/`) — landing page. **5** full-width tabs: Awaiting First Program (red), New Training Phase Due (gold), Active Programs (green), **Program Updates** (red badge; **placeholder** — cohort filter empty until rules are defined), Holiday Programs & Holds (blue). Member lists are derived from the same `fetchMembers` load as before (not per-tab Supabase queries). **Row click (Awaiting / New Training Phase Due / Active / Program Updates):** opens **`MemberDetailModal`** (`features/queue/MemberDetailModal.tsx`) — overlay with **Client Profile** + **Movement & Benchmarks** matching Intake (`ClientProfileCards`, `MovementBenchmarksSection`). Data loads via **`useIntakeData`** (`hooks/useIntakeData.ts`; 5 queries without history). **Open Program Editor** goes to `/program/:memberId`; Escape or backdrop closes the modal. **Holiday Programs & Holds** tab: unchanged — click selects member and navigates to `/holiday`. Shared intake UI: `components/ui/IntakeCards.tsx` (`FormField`, `RagBadge`, `BenchmarkCard`), `lib/intakeTypes.ts`, `lib/intakeProfile.ts` (`buildMemberProfile`). Active Programs / queue semantics unchanged (no FK note still applies: `supabase/migrations/20260408000001_fk_programming_generated_member.sql`). **Holiday** tab layout: dual-column holds + holiday program cards per member as before.
