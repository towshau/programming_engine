...
## Tables (14 total, all created and applied)

| Table | Purpose | Key columns |
|-------|---------|-------------|
| member_programming_notes | Session / physio–driven programming change requests per member | id, member_id, member_name, staff_logged_id, staff_name, modification (`programming_note_modification`: Injury / Pain, Physio Advice, Other), submission_date, details, **implemented** (boolean, default false — coach marks done in Program Editor). Migration: `supabase/migrations/20260414120000_add_implemented_to_member_programming_notes.sql`. **fetchMembers** loads recent rows **including implemented** (cap e.g. 8000, by `submission_date`) for Program Updates history / grey rows. Per-member arrays: **unactioned first**, **Injury / Pain** before other types, then **submission_date** desc. |
...
- Routes: `/` → ClientQueue, `/intake[/:memberId]` → Intake, `/program[/:memberId]` → ProgrammingEngine, `/holiday` → HolidayPrograms, `/360` → ThreeSixty (coach performance metrics)
- `MemberSidebar` — collapsible (chevron toggle; collapsed = 40px icon rail, expanded = 288px). Used on Intake and ProgrammingEngine pages. Not used on ClientQueue (has its own inline list). **Program Editor (`source=programming`):** under search, **filter by programming coach** (`member_programs.programming_coach_id` / `MemberWithCoach.programming_coach_id`); each row shows **`PC: First Last`** (small muted text). Intake sidebar unchanged (no PC filter row).

**Pages:**
- **ClientQueue** (`/`) — landing page. **5** full-width tabs: Awaiting First Program (red), New Training Phase Due (gold), Active Programs (green), **Program Updates** (red tab badge = members with **unactioned** notes; list includes members with **any** note, fully actioned rows **greyed**), Holiday Programs & Holds (blue). `fetchMembers` loads recent **`member_programming_notes`** (implemented + not) for queue/history. **Row click:** **Awaiting First Program** → **`MemberDetailModal`** (Client Profile + Movement & Benchmarks + Open Program Editor). **New Training Phase Due / Active Programs / Program Updates** → **`/program/:memberId`** directly. **Holiday** tab → `/holiday`. **Program Updates:** sorted **Urgent** (`Injury / Pain`) unactioned first, then other unactioned, then all-actioned; meta shows **Urgent/Medium** + modification + date; **pagination** (20 per page). Shared intake UI: `IntakeCards`, `intakeTypes`, `intakeProfile`. **ProgrammingNotesPanel**: same sort; **implemented** notes stay visible **greyed**; optional **pagination** (20) in expanded list; **Mark as implemented** refreshes `fetchMembers`.
...
- **360** (`/360`) — profile menu. Coach performance dashboard: **Winning client results** (`coach_wcr_logging` + coach name), **Renewals in cycle** (`member_memberships` with `member_database` + primary/handoff coach names; `end_date` filtered to selected **Renewal cycle**), **Renewal points** (`member_renewal_meta` + coach name). Uses header **coach** filter (UUID on `coach_id`; renewals also match `handoff_coach_id`). **Renewal cycle** = 6-month periods Nov1–Apr 30 and May 1–Oct 31 (`features/three-sixty/cycleUtils.ts`); dropdown defaults to current half-year. Data hook: `features/three-sixty/useThreeSixty.ts`; page: `pages/ThreeSixtyPage.tsx`. **TODO (cleanup):** Refine the three on-page tables (column sets, typings, Supabase embeds/RLS assumptions) once usage settles; keep this bullet and routes accurate in **this** file, and mirror the narrative into the Coach OS wiki / canonical repo: https://github.com/Lockeroom-Gym/coachOS

- **Coach selector** — top bar dropdown (`All Coaches` = no filter). Options: active `staff_database` rows whose `role` is one of Coach, Advanced Coach, Gym Manager, Senior Coach, Casual Coach, Head of Exercise. Selected coach ID persisted in `localStorage` (`lr-selected-coach-id`) across refresh. Filters member list by **`member_memberships.handoff_coach_id ?? coach_id`** (aligns with Intake logic and Retool). Gym / membership stage come from active `member_memberships` rows. **Programming coach on each member** (`programming_coach_id` for PC label + sidebar filter): resolved from **`member_programs`** by matching **`membership_id`** to the member’s primary membership (latest `start_date` in the cohort query), then tie-breaking duplicates by non-`inactive` stage, **`updated_at`**, **`due_date`**, **`id`**; if no `member_programs` row, falls back to **`member_memberships.programming_coach_id`**. **`sessions_per_week`** in the queue/UI comes from the latest **`programming_generated`** row for that member, not `member_programs`.
- **Member sidebar** — debounced search, status chips, **programming-coach filter** (Program Editor only), member name + gym badge + **PC** line; click to load program.
