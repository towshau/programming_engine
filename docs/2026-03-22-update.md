...
13. **Weekly batch generation** — `run_weekly_batch.py` runs on Monday (GitHub Actions cron). Finds members whose programs are due within 8 days or awaiting a program. Skips members with a recent generation. Runs the full pipeline for each.
...
| Weekly batch generation | `python tools/run_weekly_batch.py` (or GitHub Actions Monday 09:00 UTC) | repo root |
...
## Remaining to-do list (CEO summary)

These items are **not yet automated in production**. Everything above describes what we have; this section is what is **still to build or schedule**.
...
### Recommended day/time: browser sync vs weekly generation

- **Weekly program generation is already scheduled:** GitHub runs every **Monday at 09:00 UTC** (automated pipeline: normalize → auto-exclusions → generate for due members). That is **Monday morning**, not Sunday.
- **Goal:** Run the **TeamBuildr → Supabase** scrape **after** coaches usually finish editing TeamBuildr for the week, but **before** Monday **09:00 UTC**, so the generator sees the latest live programs.

**Practical options:**

| Option | Idea | Trade-off |
|--------|------|-----------|
| **A (tightest fit)** | **Sunday evening (Australia)** or **Monday morning before 09:00 UTC** | Best match to Monday generation; weekend TB changes are captured. |
| **B (earlier in the week)** | e.g. **Friday** | Simpler operationally, but any coach changes **Saturday–Monday morning** would **not** be in Supabase before the Monday generator runs — use only if weekend TB edits are rare. |

**Recommendation for Lockeroom:** Prefer **Sunday evening (local)** or a **scheduled run Monday 06:00–08:00 UTC** (after confirming no coach workflow depends on Monday-morning TB edits before 09:00 UTC). Adjust once we know typical coach behaviour.

**Future state:** Add a GitHub Action (or a dedicated runner with Playwright) that runs the pull sync on that schedule, **before** the existing Monday 09:00 UTC workflow — or merge ordering so “pull from TeamBuildr” is **step 0** before normalize/generate on the same machine.
