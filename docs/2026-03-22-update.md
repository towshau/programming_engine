...
|----------|------|------------|----------------|
| **1** | **Weekly automation for TeamBuildr → Supabase (browser scraping)** | *(Removed)* The Playwright batch sync has been removed. The pipeline now fully relies on the generated baseline in `programming_generated`. |
| **2** | **Supabase → TeamBuildr upload script (Playwright)** | *(Removed)* Automated push via Playwright is also de-scoped in favor of manual verification or an official API if one becomes available. |

### When to run the browser sync vs weekly generation

*(Note: The automated browser sync has been removed. Below is historical context for the weekly generation timing.)*
...
*(Note: The Playwright-based scraper was removed due to reliability issues with the TeamBuilder UI. Syncing is now a manual admin process or relies strictly on the `programming_generated` baseline.)*
...
*(Historical context: The first sync run with the now-removed Playwright scraper.)*
...
| Per-row `uploaded_to_teambuildr` flag | Tracks which programs have been manually synced to TeamBuilder |
...
| Sync one member from TeamBuilder | *(Removed)* | *(Removed)* |
| Batch sync all members | *(Removed)* | *(Removed)* |
| Batch sync (dry run) | *(Removed)* | *(Removed)* |
| Batch sync (reset + restart) | *(Removed)* | *(Removed)* |
...
- **Manual TeamBuilder Sync** — Continue relying on manual syncs from TeamBuilder.
...
- **Trial weekly cycle**: Generate → Review → Upload → Feedback.
