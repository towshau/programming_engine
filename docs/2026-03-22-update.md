...
- **Weekly program generation (already live):** GitHub runs every **Monday at 09:00 UTC** (normalize → auto-exclusions → generate for due members). **Not** Sunday.
- **Goal:** Run the **TeamBuildr → Supabase** scrape **after** coaches usually finish editing TeamBuildr for the week, but **before** Monday **09:00 UTC**.
...
| **B (earlier in the week)** | e.g. **Friday** | Simpler operationally, but coach changes **Saturday–Monday morning** would **not** be in Supabase before the Monday generator — use only if weekend TB edits are rare. |

**Recommendation:** Prefer **Sunday evening (local)** or **Monday 06:00–08:00 UTC**, then wire **automated** Playwright on that schedule as **step 0** before the existing Monday 09:00 UTC job (or a combined pipeline).
